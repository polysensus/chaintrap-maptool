...
class Generator:
    ...
    def _corridor_lmerge_spur00(self, icor, jcor, iside, jside):
        """
        iside = jside == 0
                  + i2
                  |
    ...
    def _corridor_lmerge_spur01(self, icor, jcor, iside, jside):
        """
        iside = 0, jside == 1
                  + i2
                  |
    ...
    def _corridor_lmerge_spur10(self, icor, jcor, iside, jside):
        """
        iside = 1, jside == 0
                  + i0
                  |
    ...
    def _corridor_lmerge_spur11(self, icor, jcor, iside, jside):
        """
    ...
    def _corridor_lmerge_spur(self, icor, jcor, iside, jside):
        ...
        # remove the reference to the corridor segment we are deleting in order to form the spur
        # assert cj.joins[jside] == iroom_remove
        self.rooms[cj.joins[jside]].detach_corridor(jcor)

        # attach the corridor to the new intersection. Note we are removing
        # jside, so we update the other side of the join.
        cj.joins[1 - jside] = iinter
        ...
        inter.corridors[RoomSide.opposite(cj.join_sides[jside])].append(jcor)

        # attach the new corridor to the clipped short leg. Note that
        # this attachment is awkward - its at 90'

        if cj.join_sides[jside] in [RoomSide.NORTH, RoomSide.SOUTH]:
        ...
        # we are removing the room at jside and attaching
        # join_sides[jside] is attached to the room we are keeping. The room in
        # join_sides[1 - jside] is now the intersection. We know that the room
        # side of the intersection is *opposite* the room we are keeping
        cj.join_sides[1 - jside] = RoomSide.opposite(cj.join_sides[jside])
