...
# Original filename: ecvrf_edwards25519_sha512_elligator2_test.py
...
test_dict['e'] = (ec.PRIME - 1).to_bytes(32, 'little')  # -1
...
ec.test_dict = test_dict

p_status, pi_result = ec.ecvrf_prove(sk, alpha_string)
assert p_status == "VALID" and pi_result == test_dict['pi_string']
b_status, beta_result = ec.ecvrf_proof_to_hash(pi_result)
assert b_status == "VALID" and beta_result == test_dict['beta_string']
Y = ec.get_public_key(sk)
valid_result, valid_beta = ec.ecvrf_verify(Y, pi_result, alpha_string)
...
for index in range(5):
    ...
    Y = ec.get_public_key(sk)
    p_status, pi_string = ec.ecvrf_prove(sk, alpha_string)
    b_status, beta_string = ec.ecvrf_proof_to_hash(pi_string)
    valid_result, valid_beta = ec.ecvrf_verify(Y, pi_string, alpha_string)
    ...
    bad_beta = ec.ecvrf_proof_to_hash(bad_pi)
    valid_result, valid_beta = ec.ecvrf_verify(Y, bad_pi, alpha_string)
...
for index in range(3):
    ...
    print("\nY = ec.get_public_key(sk)")
    print("p_status, pi_string = ec.ecvrf_prove(sk, alpha_string)")
    print("b_status, beta_string = ec.ecvrf_proof_to_hash(pi_string)")
    print("valid_result, valid_beta = ec.ecvrf_verify(Y, pi_string, alpha_string)")
